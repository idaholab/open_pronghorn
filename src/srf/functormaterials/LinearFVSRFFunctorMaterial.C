...
LinearFVSRFFunctorMaterial::validParams()
{
  ...
  params.addClassDescription("Creates functors for the Single Rotating Frame approach");

  // Metacenter origin
  params.addRequiredParam<RealVectorValue>(
      "mc_origin", "Vector Coordinates of the metacenter reference frame w.r.t the origin");

  // Inclination angle parameters
  params.addParam<MooseEnum>(
      "SRF_input_mode", MooseEnum("fixed pitch_yaw_roll", "pitch_yaw_roll"), "SRF input mode");
  ...
  params.addParam<MooseFunctorName>(
      "pitch_omega_fixed", 0.0, "Constant pitch angular velocity in rad/s");
  params.addParam<MooseFunctorName>(
      "yaw_omega_fixed", 0.0, "Constant yaw angular velocity in rad/s");
  params.addParam<MooseFunctorName>(
      "roll_omega_fixed", 0.0, "Constant roll angular velocity in rad/s");

  // Angular acceleration parameters
  params.addParam<MooseFunctorName>(
      "pitch_omegadot_fixed", 0.0, "Constant pitch angular acceleration in rad/s^2");
  params.addParam<MooseFunctorName>(
      "yaw_omegadot_fixed", 0.0, "Constant yaw angular acceleration in rad/s^2");
  params.addParam<MooseFunctorName>(
      "roll_omegadot_fixed", 0.0, "Constant roll angular acceleration in rad/s^2");
...
LinearFVSRFFunctorMaterial::LinearFVSRFFunctorMaterial(const InputParameters & params)
  : FunctorMaterial(params),
    ...
    _pitch_angle_fixed(isParamValid("pitch_angle_fixed") ? &getFunctor<Real>("pitch_angle_fixed")
                                                         : nullptr),
    _yaw_angle_fixed(isParamValid("yaw_angle_fixed") ? &getFunctor<Real>("yaw_angle_fixed")
                                                     : nullptr),
    _roll_angle_fixed(isParamValid("roll_angle_fixed") ? &getFunctor<Real>("roll_angle_fixed")
                                                       : nullptr),
    ...
    _roll_pha(getFunctor<Real>("roll_pha"))
{
  ...
  addFunctorProperty<RealVectorValue>("r_mc",
  ...
  if (_SRF_input_mode == "fixed")
  {
    ...
    addFunctorProperty<Real>("pitch_angle",
                             [this](const auto & r, const auto & t) -> Real
                             { return (*_pitch_angle_fixed)(r, t) * (libMesh::pi / 180.0); });

    addFunctorProperty<Real>("yaw_angle",
                             [this](const auto & r, const auto & t) -> Real
                             { return (*_yaw_angle_fixed)(r, t) * (libMesh::pi / 180.0); });

    addFunctorProperty<Real>("roll_angle",
                             [this](const auto & r, const auto & t) -> Real
                             { return (*_roll_angle_fixed)(r, t) * (libMesh::pi / 180.0); });

    addFunctorProperty<RealVectorValue>("omega_brf",
                                        [this](const auto & r, const auto & t) -> RealVectorValue
                                        {
                                          return RealVectorValue(_roll_omega_fixed(r, t),
                                                                 _pitch_omega_fixed(r, t),
                                                                 _yaw_omega_fixed(r, t));
                                        });

    // Scalar aliases
    addFunctorProperty<Real>("omega_roll",
                             [this](const auto & r, const auto & t) -> Real
                             { return _roll_omega_fixed(r, t); });

    addFunctorProperty<Real>("omega_pitch",
                             [this](const auto & r, const auto & t) -> Real
                             { return _pitch_omega_fixed(r, t); });

    addFunctorProperty<Real>("omega_yaw",
                             [this](const auto & r, const auto & t) -> Real
                             { return _yaw_omega_fixed(r, t); });

    addFunctorProperty<RealVectorValue>("omega_dot_brf",
                                        [this](const auto & r, const auto & t) -> RealVectorValue
                                        {
                                          return RealVectorValue(_roll_omegadot_fixed(r, t),
                                                                 _pitch_omegadot_fixed(r, t),
                                                                 _yaw_omegadot_fixed(r, t));
                                        });

    addFunctorProperty<Real>("omega_dot_roll",
                             [this](const auto & r, const auto & t) -> Real
                             { return _roll_omegadot_fixed(r, t); });

    addFunctorProperty<Real>("omega_dot_pitch",
                             [this](const auto & r, const auto & t) -> Real
                             { return _pitch_omegadot_fixed(r, t); });

    addFunctorProperty<Real>("omega_dot_yaw",
                             [this](const auto & r, const auto & t) -> Real
  ...
  else if (_SRF_input_mode == "pitch_yaw_roll")
  {
    ...
    auto eval_angle = [this, deg_to_rad](const Moose::Functor<Real> & amp_deg,
    ...
    auto eval_rate = [this, deg_to_rad](const Moose::Functor<Real> & amp_deg,
                                        const Moose::Functor<Real> & per,
                                        const Moose::Functor<Real> & pha_deg,
                                        const std::string & label,
                                        const auto & r,
    ...
    auto eval_accel = [this, deg_to_rad](const Moose::Functor<Real> & amp_deg,
                                         const Moose::Functor<Real> & per,
                                         const Moose::Functor<Real> & pha_deg,
                                         const std::string & label,
                                         const auto & r,
    ...
    auto omega_brf_value = [this, eval_angle, eval_rate](const auto & r,
                                                         const auto & t) -> RealVectorValue
    {
      using std::cos;
      using std::sin;
      ...
      const RealVectorValue omega_brf(roll_dot + sin(yaw) * pitch_dot,
