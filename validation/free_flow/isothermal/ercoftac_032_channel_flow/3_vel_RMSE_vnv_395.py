...
class TestCase(ValidationCase):
    def initialize(self):
        ...
        ercoftac_U_mean = np.interp(
            1.0 - moose_csv["y"], ercoftac_csv["y"], ercoftac_csv["Umean"]
        )

        ### Compute friction velocity u_tau (rho = 1)
        nu = 2.0 / 14000  # Bulk Reynolds number
        ...
        moose_lsfv_ref_csv = pd.read_csv(
            "gold/Ret395_MOOSE_LSFV_k-epsilon_test_csv_0002.csv"
        )
    ...
    def validParams():
        ...
        params.addRequiredParam(
            ...
            "The lower bound for the RMS on the velocity profile",
        )
        params.addRequiredParam(
            "validation_upper_bound",
            "The upper bound for the RMS on the velocity profile",
        )
        return params
    ...
    def testValidation(self):
        ...
        self.addScalarData(
            key="y_plus",
            value=self.yplus_rel_diff,
            description="Relative difference on outlet wall adjacent cell Y plus",
            units="-",
