#===============================================================================
# Chen test – air + aerosol drift-flux
#===============================================================================
...
lambda_mfp = 0.066e-6 # mean free path [m] (air)
Sc_t_aero = 1.0       # turbulent Schmidt for aerosol
...
[Variables]
  ...
  # --- Aerosol active scalar (volume or mass fraction)
  [aerosol]
    type = MooseLinearVariableFVReal
    solver_sys = aerosol_system
  []
[]
...
[LinearFVKernels]
  ...
  inactive = 'u_time v_time w_time TKE_time TKED_time aerosol_time'
  ...
  #---------------------------------------------------------------------------
  # AEROSOL TRANSPORT (active scalar + drift flux + diffusion)
  #---------------------------------------------------------------------------
  ...
  # Convection with the carrier velocity
  [aerosol_advection]
    type = LinearFVScalarAdvection
    variable = aerosol
  []

  # Drift-flux source term: gravity + (potential) thermophoresis
  [aerosol_drift_flux]
    type = LinearFVAerosolDriftFlux
    variable = aerosol
  ...
  [aerosol_diffusion]
    type = LinearFVDiffusion
    variable = aerosol
    diffusion_coeff = D_eff   # Aux with full D_B + turbulent part (see AuxKernels)
  []
[]
...
[LinearFVBCs]
  ...
  #---------------------------------------------------------------------------
  # AEROSOL BOUNDARIES (Chen case)
  #---------------------------------------------------------------------------

  # Inlet aerosol: continuous injection at normalized concentration = 1
  [inlet_aerosol]
    type = LinearFVAdvectionDiffusionFunctorDirichletBC
    boundary = 'inlet'
    variable = aerosol
  ...
  # Deposition at walls (Lai & Nazaroff / Gray / drift-flux)
  [walls_aerosol_dep]
    type = LinearFVAerosolDepositionBC
    boundary = 'wall'
    variable = aerosol
...
[Executioner]
  ...
  active_scalar_systems = 'aerosol_system'
  active_scalar_l_abs_tol = 1e-9
  active_scalar_l_tol     = 1e-9
