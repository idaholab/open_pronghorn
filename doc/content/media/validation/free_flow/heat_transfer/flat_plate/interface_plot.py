...
def read_moose(path):
    ...
    if len(t_cols) != 1:
        raise ValueError(f"Expected exactly one temperature column starting with 'T_' in {path}, got {t_cols}")
    df = df.rename(columns={t_cols[0]: "T"})
    # Keep standard columns if present; otherwise keep what exists
    keep_cols = [c for c in ["T", "x"] if c in df.columns]
...
def analytic_approximate_solution_bl(x,y):
    ...
    if (y < delta_T and y > 0):
        T_f = T_w + 3.0 / 2.0 * (T_inf - T_w) / delta_T * y - 1.0 / 2.0 * (T_inf - T_w) / np.pow(delta_T, 3.0) * np.pow(y, 3.0)
...
moose_fluid_df = read_moose("../../../../../../../validation/free_flow/heat_transfer/flat_plate/gold/cht_rob-rob_out_interface_temp_0001.csv")
moose_fluid_df = moose_fluid_df.sort_values("x")
if (print_commercial_cfd):
    reference_cfd_df = read_moose("../../../../../../../validation/free_flow/heat_transfer/flat_plate/gold/reference_cfd_horizontal.csv").sort_values("x")
...
if (print_commercial_cfd):
    ax.plot(reference_cfd_df["x"],reference_cfd_df["T"], "--", label="Commercial CFD tool")
