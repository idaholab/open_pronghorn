!config navigation breadcrumbs=False scrollspy=False

# OpenPronghorn class=center style=font-weight:200;font-size:220%

!media media/open_pronghorn_logo.png
       class=landing-logo
       style=display:block;margin-left:auto;margin-right:auto;width:30%;
       alt=OpenPronghorn logo

!style halign=center fontsize=120% color=0.35 0.35 0.35
Coarse-mesh computational fluid dynamics built on the MOOSE framework.

!row!
!col! small=12 medium=4 large=4 icon=waves
### Validated Flow Physics class=center style=font-weight:200;

!style halign=center

- Steady-state, transient, laminar turbulent cases
- Documented verification data sets you can compare against immediately
- Ready-to-run inputs for rapid regression testing
!col-end!

!col! small=12 medium=4 large=4 icon=flash_on
### Built on MOOSE class=center style=font-weight:200;

!style halign=center

- Access to the complete library of MOOSE kernels and materials
- Shareable workflows that follow familiar syntax and execution patterns
- Scales from laptops to leadership-class clusters
!col-end!

!col! small=12 medium=4 large=4 icon=group
### Quality Driven class=center style=font-weight:200;

!style halign=center

- Consistent coding standards and automated testing
- Traceability from requirements to validation evidence
- Automated performance testing
!col-end!
!row-end!
